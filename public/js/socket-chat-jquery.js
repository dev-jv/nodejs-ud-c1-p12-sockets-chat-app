...
let mainHeader= $('.chat-main-header');

const renderRoom = (person) => {

    let html = '';

    html += '<div class="p-20 b-b">';
    html += '   <h3 class="box-title">Chat room <small>' + person.room + '</small></h3>';
    html += '</div>';

    mainHeader.html(html);
};

const renderUsers = (persons, x) => {
    ...
    if(x) {
        html += '<li>';
        html += '    <a data-id="' + x.id + '" href="javascript:void(0)"><img src="assets/images/users/1.jpg" alt="user-img" class="img-circle"> <span>' + x.name + '<small class="text-danger">offline</small></span></a>';
        html += '</li>';
    }
